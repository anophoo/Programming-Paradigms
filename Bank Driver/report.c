...
{
  sem_wait(&(bank->lockForReportTransfer)); // რამდენს შეუძლია ტრანსფერ-რეპორტის გაკეთება
  ...
  sem_post(&(bank->lockForReportTransfer)); // სხვასაც შეუძლია ტრანსფერ-რეპორტის გაკეთება უკვე
  return 0;
}
...
{
  sem_wait(&(bank->forCheck));  // ატომური ოპერაციაა და ეს მაქინაციები საჭიროა
  bank->numberWorkersHasToFinish--;   // ერთი შემოსულია და შევამცირე რაოდენობა იმათი, ვისაც მოცდა მოუწევს
  /* ბოლოა თუ არა ეს სრედი */
  if(bank->numberWorkersHasToFinish != 0) {   // გავაგრძელოთ, თუ ბოლოა

    // შემოწმების საშუალება სხვასაც მივცეთ ანუ
    sem_post(&(bank->forCheck));
    // და გამოთქვათ მზადყოფნა მე-2 დღის დაწყებისათვის
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      // We've run out of report storage for the bank
      bank-> numberWorkersHasToFinish = numWorkers; // ეს უნდა აქ???
      sem_post(&(bank->forCheck));
      return -1;
  }
  ...
  for (int i = 0; i < numWorkers - 1; i++) { // numWorkers - 1
    sem_post(&(bank->letMeStartNextDay));
  }
  bank-> numberWorkersHasToFinish = numWorkers;   // ეს აქ უნდა???
  sem_post(&(bank->forCheck)); // ეს საიდან მოეთრა?
  return err;
}
