...
void six(string source, string target) {
...
/* finds shortest path */
void shortest(string source, string target, imdb &db, bool &isPath) {
  list<path> partialPaths; // functions a queue
      ...
      if (previouslySeenFilms.find(films[i]) == previouslySeenFilms.end()) {
        ...
        for (int j = 0; j < (int) cast.size(); j++)
        {
          if (previouslySeenActors.find(cast[j]) == previouslySeenActors.end()) {
            ...
            clone.addConnection(films[i], cast[j]);
            if (cast[j] == target)
            {
              isPath = true;
              cout << clone << endl;
...
int main(int argc, const char *argv[])
{
  ...
  while (true) {
    ...
    if (source == target) {
      ...
      bool isPath = false;
      shortest( source,  target, db, isPath);
