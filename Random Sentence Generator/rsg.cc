...
void rsg(string str, map<string, Definition>& grammar){
  Definition def = grammar[str];
  Production pr = def.getRandomProduction();
  for (Production::iterator curr = pr.begin(); curr != pr.end(); ++curr) {
    string tmp = *curr;
    if (tmp[0] != '<') {
      cout << " " << tmp;
    } else {
